...
def main():
    ...
    var_CanvasSun = GraphWin("CanvasSun", 400, 400)     # todos as novas pens criadas vão desenhar neste canvas, até que um novo canvas seja criado


    # criar pen_class default, e modificar-lhe os atributos indicados 
    var_PenTypeSun = {}
    var_PenTypeSun["pos"] = (10,10)
    var_PenTypeSun["color"] = "red"
    var_PenTypeSun["orientation"] = 0
    var_PenTypeSun["thickness"] = 1
    var_PenTypeSun["pressure"] = 0
    var_PenTypeSun["levitate"] = False


    # meter os valores desejados na pen_class criada
    penClass_set_attribute( var_PenTypeSun, 'color', "yellow"  )
    penClass_set_attribute( var_PenTypeSun, 'position', (200, 200)  )
    penClass_set_attribute( var_PenTypeSun, 'orientation', 0  )
    penClass_set_attribute( var_PenTypeSun, 'thickness', 2  )
    penClass_set_attribute( var_PenTypeSun, 'pressure', 1  )



    var_penSun = Pen(  var_PenTypeSun )


    pen_action(var_CanvasSun, var_penSun, "down")


    var_i = int(0)
    while( (var_i < 12) ):
        pen_action(var_CanvasSun, var_penSun, "forward", value=80)
        pen_action(var_CanvasSun, var_penSun, "backward", value=80)
        pen_action(var_CanvasSun, var_penSun, "right", value=30)
        var_i = ((var_i + 1))


    pen_action(var_CanvasSun, var_penSun, "up")


    pen_action(var_CanvasSun, var_penSun, "down")


    var_penSun.set_attribute( 'color', "orange"  )


    pen_action(var_CanvasSun, var_penSun, "left", value=90)


    var_j = int(0)
    while( (var_j < 36) ):
        pen_action(var_CanvasSun, var_penSun, "forward", value=10)
        pen_action(var_CanvasSun, var_penSun, "right", value=10)
        var_j = ((var_j + 1))
    ...
    var_CanvasSun.setBackground("cyan")
    try:
        var_Win.getMouse()
        var_Win.close()
    except:
