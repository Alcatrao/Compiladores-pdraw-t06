...
def main():
    ...
    var_height = float(800)



    var_Canvas1 = GraphWin("Canvas1", var_width, var_height)     # todos as novas pens criadas vão desenhar neste canvas, até que um novo canvas seja criado
    ...
    penClass_set_attribute( var_PenType2, 'color', "blue"  )
    penClass_set_attribute( var_PenType2, 'position', ((conversion("int", var_width) // 2), (conversion("int", var_height) // 2))  )
    ...
    def var_dor(var_cena):
        var_dor_qualidade_cena = str("dor qualidade cena")
        if((var_cena > 0)):
            #printar variável indicada
            print(var_cena)
            pause((1000000))
            if((var_cena > 4)):
                var_Win.setBackground("#ead9e0")
            if((var_cena == 4)):
                var_Win.setBackground("#fbecff")
            if((var_cena == 3)):
                var_Win.setBackground("#f5d2ff")
            if((var_cena == 2)):
                var_Win.setBackground("#f1f1f1")
            if((var_cena == 1)):
                var_Win.setBackground("#ffffff")
            var_dor((var_cena - 1))



    var_Canvas1.setBackground("#FFA800")
    ...
    pen_action(var_Win, var_p2, "down")


    pen_action(var_Win, var_p2, "forward", value=10)


    pen_action(var_Win, var_p2, "right", value=144, units='º')
